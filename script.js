...
const translitMap = {
    A: 'А', B: 'Б', V: 'В', G: 'Г', D: 'Д', E: 'Е', EO: 'Ё', ZH: 'Ж', Z: 'З', I: 'И', Y: 'Й',
    K: 'К', L: 'Л', M: 'М', N: 'Н', O: 'О', P: 'П', R: 'Р', S: 'С', T: 'Т', U: 'У', F: 'Ф',
    H: 'Х', C: 'Ц', CH: 'Ч', SH: 'Ш', SCH: 'Щ', YU: 'Ю', YA: 'Я', J: 'ДЖ', W: 'В', Q: 'КВ', KY: 'КИЙ'
};
...
function extractBetweenLabels(text, startLabel, endLabel) {
    ...
    return startIndex !== -1 && endIndex !== -1 ? text.substring(startIndex + startLabel.length, endIndex).trim() : null;
}
...
function extractCityAndAddress(text, fullText) {
    ...
    if (!isCityFound) cityOutput = `<span class="attention">${transliteratedCity}</span>`;
    if (!isAddressFound) addressOutput = `<span class="attention">${transliteratedAddress}</span>`;
    return `${cityOutput}, ${addressOutput}`;
}

function transliterateToRussian(text) {
    text = text.toUpperCase();
    return text.replace(/(SH|CH|SCH|KY|YU|YA)/g, match => translitMap[match] || match)
...
function highlightKeywords(text) {
    const keywords = ["ВНИМАНИЕ", "МУЛЬТИМЕРЧАНТ", "ЭЛЕКТРОННЫЕ СЕРТИФИКАТЫ", "ОБРАТНЫЙ ЭКВАЙРИНГ", "ОЭ", "ЭС"];
